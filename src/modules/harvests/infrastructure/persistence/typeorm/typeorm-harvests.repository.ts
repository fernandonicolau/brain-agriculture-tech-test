...
import {
  FindAllHarvestsParams,
  HarvestsRepositoryPort,
} from '@/modules/harvests/application/ports/harvests.repository';

@Injectable()
export class TypeOrmHarvestsRepository implements HarvestsRepositoryPort {
  constructor(
  ...
  findAll({ page, limit, farmId }: FindAllHarvestsParams): Promise<[Harvest[], number]> {
